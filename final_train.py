...
class DivisionStrategyFinal:
    ...
    def __init__(self, camera, world_size, gpu_ids, division_pos, gpu_for_this_camera_tilelr):
        ...
        assert division_pos[-1] == utils.TILE_Y*utils.TILE_X, "The last element of division_pos must be equal to the total number of tiles."
    ...
    def get_compute_locally(self):
        ...
        tile_ids_l, tile_ids_r = self.division_pos[rank], self.division_pos[rank+1]
    ...
    def get_extended_compute_locally(self):
        ...
        tile_ids_l, tile_ids_r = self.division_pos[rank], self.division_pos[rank+1]
        tile_l = max(tile_ids_l-utils.TILE_X-1, 0)
        tile_r = min(tile_ids_r+utils.TILE_X+1, utils.TILE_Y*utils.TILE_X)
        extended_compute_locally = torch.zeros(utils.TILE_Y*utils.TILE_X, dtype=torch.bool, device="cuda")
        extended_compute_locally[tile_l:tile_r] = True
        extended_compute_locally = extended_compute_locally.view(utils.TILE_Y, utils.TILE_X)
...
class DivisionStrategyHistoryFinal:
    def __init__(self, dataset, world_size, rank):
        ...
        for camera in dataset.cameras:
            self.accum_heuristic[camera.uid] = torch.ones((utils.TILE_Y*utils.TILE_X, ), dtype=torch.float32, device="cuda", requires_grad=False)
...
def start_strategy_final(batched_cameras, strategy_history):
    ...
    n_tiles_per_image = utils.TILE_Y * utils.TILE_X
    ...
    for i in range(1, len(division_pos)-1):
        if (division_pos[i] % n_tiles_per_image + args.border_divpos_coeff*utils.TILE_X >= n_tiles_per_image):
            division_pos[i] = division_pos[i] // n_tiles_per_image * n_tiles_per_image + n_tiles_per_image
        elif division_pos[i] % n_tiles_per_image - args.border_divpos_coeff*utils.TILE_X <= 0:
            division_pos[i] = division_pos[i] // n_tiles_per_image * n_tiles_per_image
    for i in range(0, len(division_pos)-1):
        assert division_pos[i] + 2*utils.TILE_X < division_pos[i+1], "The division_pos must be large enough to boundary case error."

    # we should set right=True, because we each gpu compute equal number of tiles at the first step.
    # print("rank ", utils.GLOBAL_RANK, "division_pos", division_pos) # 
...
def finish_strategy_final(batched_cameras, strategy_history, batched_strategies, batched_statistic_collector):
    ...
    for camera_id, (camera, strategy) in enumerate(zip(batched_cameras, batched_strategies)):
        new_heuristic = torch.zeros((utils.TILE_Y*utils.TILE_X, ), dtype=torch.float32, device="cuda")
...
def get_coverage_y_min_max(tile_ids_l, tile_ids_r, window_size=11):
    first_tile_y = tile_ids_l // utils.TILE_X
    first_pixel_y = first_tile_y * utils.BLOCK_Y
    last_tile_y = tile_ids_r // utils.TILE_X
    if tile_ids_r % utils.TILE_X == 0:
    ...
    return coverage_min_y, coverage_max_y

def get_coverage_y_min(tile_ids_l, window_size=11):
    first_tile_y = tile_ids_l // utils.TILE_X
    first_pixel_y = first_tile_y * utils.BLOCK_Y
    coverage_min_y = max(first_pixel_y-window_size, 0)
    return coverage_min_y

def get_coverage_y_max(tile_ids_r, window_size=11):
    last_tile_y = tile_ids_r // utils.TILE_X
    if tile_ids_r % utils.TILE_X == 0:
        last_pixel_y_plus1 = min(last_tile_y * utils.BLOCK_Y, utils.IMG_H)
    else:
