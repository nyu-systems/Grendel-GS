...
def training(dataset_args, opt_args, pipe_args, args, log_file):
    ...
    for iteration in range(start_from_this_iteration, opt_args.iterations + 1, args.bsz):
        ...
        if args.backend == 'gsplat':
            batched_screenspace_pkg = gsplat_distributed_preprocess3dgs_and_all2all_final(batched_cameras, gaussians, pipe_args, background,
...
def training_report(iteration, l1_loss, testing_iterations, scene : Scene, pipe_args, background, backend):
    args = utils.get_args()
    ...
    if len(testing_iterations) > 0 and utils.check_update_at_this_iter(iteration, utils.get_args().bsz, testing_iterations[0], 0):
        ...
        validation_configs = ({'name': 'test', 'cameras' : scene.getTestCameras()}, 
                            {'name': 'train', 'cameras' : [scene.getTrainCameras()[idx*args.llffhold % len(scene.getTrainCameras())]
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx in range(1, num_cameras+1, args.bsz):
                    ...
                    batched_cameras = eval_dataset.get_batched_cameras(num_camera_to_load)
                    batched_strategies, gpuid2tasks = start_strategy_final(batched_cameras, strategy_history)
