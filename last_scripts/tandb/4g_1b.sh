...
# bash /global/homes/j/jy-nyu/gaussian-splatting/last_scripts/tandb/4g_1b.sh
...
for i in {0..3}; do
    ...
    for bsz in ${BSZ[@]}; do
        torchrun --standalone --nnodes=1 --nproc-per-node=4 /global/homes/j/jy-nyu/gaussian-splatting/train.py \
            ...
            --iterations 30000 \
            --log_interval 250 \
            --log_folder /pscratch/sd/j/jy-nyu/tmp/4g1b/$scene \
            --model_path /pscratch/sd/j/jy-nyu/tmp/4g1b/$scene \
