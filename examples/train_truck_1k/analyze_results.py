...
def extract_from_tandb_scene(folder):
    ...
    json.dump(results, open(os.path.join(folder, "tandb_scenes.json"), "w"), indent=4)
    print("Generated tandb_scenes.json for ", folder)
...
def plot_release_tandb(expe_folder_base):
    ...
    for expe_set in expe_sets:
        extract_from_tandb_scene(f"{expe_folder_base}/{expe_set}/")
    ...
    df = pd.DataFrame(columns=["Configuration", "7k Training Time", "7k test PSNR", "30k Training Time", "30k test PSNR"])

    iterations_to_check = ["7000", "30000"]

    for scene in all_scenes:
        for i, expe in enumerate(["4 GPU + Batch Size=8"]):
        # for i, expe in enumerate(["1 GPU + Batch Size=1", "4 GPU + Batch Size=8", "4 GPU + Batch Size=16"]):
            row = [scene + " + " + expe]
            expe_folder = os.path.join(expe_folder_base, expe_sets[i])
            results = json.load(open(os.path.join(expe_folder, "tandb_scenes.json"), "r"))

            for iter in iterations_to_check:
                row.append(convert2readable(results[scene][iter]["running_time"]))
                row.append(results[scene][iter]["psnr"])
            df.loc[len(df)] = row
    df.to_markdown(os.path.join(expe_folder_base, f"tandb_compare_{scene}.md"), index=False)
...
if __name__ == "__main__":
    ...
    plot_release_tandb(expe_folder_base)
