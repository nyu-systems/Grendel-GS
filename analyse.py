    ...
    for batch_size in [2**i for i in range(7)]:
                ...
                if iteration <= opt_args.iterations:
                    ...
                    if batched_parameter_gradients_pkg is not None and utils.LOCAL_RANK == 0:
                        ...
                        for name in batched_parameter_gradients_pkg.keys():
                            save_dict[name] = (batched_parameter_gradients_pkg[name], exp_avg_dict[name], exp_avg_sq_dict[name])
        ...
        if batch_size == 1:
            ...
            for iteration, grad_pkg in trajectory_dict.items():
                ...
                if (iteration+batch_size-1) not in base_trajectory_dict.keys():
                    break
                # for each gradient, compare norm and cosine similarity
                for idx, base_iteration in enumerate(base_iterations):
                    ...
                    for name, (grad, exp_avg, exp_avg_sq) in grad_pkg.items():
                        ...
                        base_grad, base_exp_avg, base_exp_avg_sq = base_trajectory_dict[base_iteration][name]
                        base_grad = base_grad.squeeze(0)
                        ...
                        grad_cosine_similarity_dict[name] = torch.nn.functional.cosine_similarity(grad[idx], base_grad, dim=-1).mean().item()
                        grad_norm_ratio_dict[name] = (torch.norm(grad[idx], dim=-1) / torch.norm(base_grad, dim=-1)).mean().item()
                        exp_avg_cosine_similarity_dict[name] = torch.nn.functional.cosine_similarity(exp_avg, base_exp_avg, dim=-1).mean().item()
                        exp_avg_norm_ratio_dict[name] = (torch.norm(exp_avg, dim=-1) / torch.norm(base_exp_avg, dim=-1)).mean().item()
                        exp_avg_sq_cosine_similarity_dict[name] = torch.nn.functional.cosine_similarity(exp_avg_sq, base_exp_avg_sq, dim=-1).mean().item()
                        exp_avg_sq_norm_ratio_dict[name] = (torch.norm(exp_avg_sq, dim=-1) / torch.norm(base_exp_avg_sq, dim=-1)).mean().item()
                        
                    grad_cosine_similarity[base_iteration] = grad_cosine_similarity_dict
                    grad_norm_ratio[base_iteration] = grad_norm_ratio_dict
                    exp_avg_cosine_similarity[base_iteration] = exp_avg_cosine_similarity_dict
                    exp_avg_norm_ratio[base_iteration] = exp_avg_norm_ratio_dict
                    exp_avg_sq_cosine_similarity[base_iteration] = exp_avg_sq_cosine_similarity_dict
                    exp_avg_sq_norm_ratio[base_iteration] = exp_avg_sq_norm_ratio_dict
            
            log_file.write("\n========================================================\ngrad stats for batch_size: {}\n\n".format(batch_size))
            for base_iteration in grad_cosine_similarity.keys():
                log_file.write("iteration: {}\n".format(base_iteration))
                for name in grad_cosine_similarity[base_iteration].keys():
                    log_file.write("param {}: grad_cosine_similarity: {}, grad_norm_ratio: {}, exp_avg_cosine_similarity: {}, exp_avg_norm_ratio: {}, exp_avg_sq_cosine_similarity: {}, exp_avg_sq_norm_ratio: {}\n".format(
                        name,
                        grad_cosine_similarity[base_iteration][name],
                        grad_norm_ratio[base_iteration][name],
                        exp_avg_cosine_similarity[base_iteration][name],
                        exp_avg_norm_ratio[base_iteration][name],
                        exp_avg_sq_cosine_similarity[base_iteration][name],
                        exp_avg_sq_norm_ratio[base_iteration][name]
                    ))
